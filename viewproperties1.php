...
<header>
    <a href="ownerlogin.html".html?owner_id=<?= $owner_id ?>" class="nav-arrow nav-left">⬅Go Back</a>
...
<footer>
    &copy; <?= date("Y") ?> RATEK ZPS. All Rights Reserved.
</footer>
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function loadProperties() {
        ...
        fetch(`fetchproperties1.php?owner_id=${ownerId}&county=${encodeURIComponent(county)}&location=${encodeURIComponent(location)}`)
            .then(res => res.json())
            .then(data => {
                    ...
                    const row = `
                        <tr>
                            ...
                            <td><button class="delete-btn" style="background:green;" 
                                onclick="window.location.href='bilo.php?property_id=${prop.id}&owner_id=${ownerId}'">BillO</button></td>
                        </tr>
                    `;
